...
void spi_writeLedDisplay(int value){
	char digitArray[4];						//Make array to store the separate digits
	sprintf(digitArray,"%d", value);		//Store the int value in the digitArray
	
	int reverseCount = numberOfDigits(value);
	...
	for(char i = 0; i <= (sizeof(digitArray)/sizeof(digitArray[0])); i++){
		spi_writeWord(reverseCount, digitArray[i]);
		reverseCount--;
		wait(500);
	}
